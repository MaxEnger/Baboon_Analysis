...
dem <- raster('C:/Users/Max/Desktop/sem3/HM_CGIS/collab_proj/r_outputs/dem_2.tif')
dem_crop <- crop(dem,modelEnv)
dem_plot <- plot(dem_crop)

```

### EVI - Vegetation Index
```{r}
evi3 <- raster('C:/Users/Max/Desktop/sem3/HM_CGIS/collab_proj/r_outputs/Homogeneity_01_05_1km_uint16.tif')
evi_crop3 <-crop(evi3,modelEnv)
evi_use <- evi_crop3/10000
evi_plot3 <- plot(evi_use)
```
...
dem_evi <- resample(dem_crop,evi_crop3,method = 'bilinear')
evi_model <- resample(evi_crop3,modelEnv,method = 'bilinear')
test <- resample(dem_evi,modelEnv,method = 'bilinear')

f_test <- crop(futureEnv,extent(test))

s <- stack(test,evi_model, modelEnv)
s2 <- stack(test,evi_model, f_test)
```
...
```{r, fig.height = 4, fig.width =6, fig.align = 'center'}
plot(EVI_cvEnv, main="EVI - coefficent of variation")
map('worldHires',xlim=c(min(olive$lon)-10,max(olive$lon)+10), ylim=c(min(olive$lat)-10,max(olive$lat)+10), fill=FALSE, add=TRUE)
points(olive$lon, olive$lat, pch="+", cex=0.2)
```
...
### Plot EVI - contrast
```{r, fig.height = 4, fig.width =6, fig.align = 'center'}
plot(EVI_conEnv, main="EVI - contrast")
map('worldHires',xlim=c(min(olive$lon)-10,max(olive$lon)+10), ylim=c(min(olive$lat)-10,max(olive$lat)+10), fill=FALSE, add=TRUE)
points(olive$lon, olive$lat, pch="+", cex=0.2)
```
...
olive.me <- maxent(s, olivetrain) # using only training data
plot(olive.me)
response(olive.me)
...
### Export Raster
```{r}
writeRaster(olive.pred, "V:/Commons/Baboon/SDM_data/olive_suitability.tif", overwrite = TRUE)
writeRaster(olive.pred, "V:/Commons/Baboon/SDM_data/olive_future_suit.tif", overwrite = TRUE)
```